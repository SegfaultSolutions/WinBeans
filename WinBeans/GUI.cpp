...
LaunchGUI (HINSTANCE hInstance) {
	...
	if (!RegisterClassExW (&wcRoot)) {
		LogMeA (FAIL, "Failed to register class.");
		return;
	}
	...
	if (!rootWnd) {
		LogMeA (FAIL, "Failed to create window.");
		return;
	}
	...
	if (!subWndLeft) {
		LogMeA (WARN, "Left Subview not created.");
		return;
	}
	...
	if (!subWndRight) {
		LogMeA (WARN, "Right Subview not created.");
		return;
	}
	...
	if (!accel) {
		LogMeA (FAIL, "Accelerator table not loaded.");
		return;
	}
...
GetBinPath (HWND oWner) {
	...
	if (!GetOpenFileNameW (&file)) {
		LogMeA (WARN, "No file path specified");
		return NULL;
	}
...
LoaderProc (WPARAM wParam, uint32_t nDrops) {
	...
	if (progBar == NULL) {
		LogMeA (FAIL, "ProgBar not created.");
		return;
	}
